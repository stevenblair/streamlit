This fork is an experiment in using Claude to perform a non-trivial task - to create a Go backend and API for Streamlit. Excuse any AI slop. This adds the `go-backend` directory which is self-contained. The only modification to the original Streamlit repo is this note in the readme. See [go-backend/README.md](go-backend/README.md) for more internal details (all generated by Claude).
