This fork is an experiment in using Claude to perform a non-trivial task - to create a Go backend and API for Streamlit. Excuse any AI slop. This adds the `go-backend` directory which is self-contained. The only modification to the original Streamlit repo is this note in the readme. See [GO_BACKEND.md](GO_BACKEND.md) for next steps and [go-backend/README.md](go-backend/README.md) for internal details (all generated by Claude). The basics appear to work, but expect that issues will occur.

The Go package can be imported as usual, and used with an API that mirrors the Python version:
